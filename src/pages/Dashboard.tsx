...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (isAdmin) {
      if (!['enrollments', 'manage_courses', 'manage_users', 'cert_requests', 'live_classes'].includes(activeTab)) {
            ...
            {!isAdmin ? (
              ...
              <>
                ...
                <button 
                  ...
                  className={cn(
                    "flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all relative whitespace-nowrap",
                    activeTab === 'live_classes' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-rose-600'
                  )}
                >
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'my_courses' && !isAdmin && (
            ...
            >
              ...
              {/* Dynamic Classroom Notifications Hub */}
              {notifications.length > 0 && (
                <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div className="flex items-center space-x-2.5">
                      <div className="relative">
                        <Bell className="h-5 w-5 text-emerald-500 animate-[bounce_1.5s_infinite]" />
                        {notifications.some(n => !n.isRead) && (
                          <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rose-500 rounded-full ring-2 ring-white"></span>
                        )}
                      </div>
                      <div>
                        <h2 className="text-base md:text-lg font-black text-slate-900 leading-tight">Classroom Notifications Hub</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Dynamic alerts for live sessions and masterclasses</p>
                      </div>
                    </div>
                    <span className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border border-emerald-100">
                      {notifications.filter(n => !n.isRead).length} New Reminder(s)
                    </span>
                  </div>

                  <div className="grid gap-3 max-h-80 overflow-y-auto pr-1 no-scrollbar">
                      ...
                      <div 
                        ...
                        className={cn(
                          ...
                          notif.isRead 
                            ...
                            : "bg-emerald-50/30 border-emerald-100 text-slate-900 shadow-sm"
                        )}
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            {!notif.isRead && (
                              <span className="bg-rose-500 text-white text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded">
                                NEW
                              </span>
                            )}
                          ...
                          <p className="font-bold text-sm text-slate-900 leading-tight">{notif.title}</p>
                          <p className="text-xs text-slate-500 leading-relaxed font-medium">{notif.message}</p>
                          <span className="text-[9px] text-slate-400 font-semibold block">
                            Alert received on: {new Date(notif.createdAt).toLocaleString()}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {!notif.isRead && (
                            <button 
                              ...
                              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] uppercase tracking-wider rounded-xl transition-all shadow-sm shadow-emerald-100"
                            >
                              Check
                            </button>
                          )}
                          <button 
                            ...
                            className="p-1 px-2.5 bg-slate-100 hover:bg-rose-50 text-slate-400 hover:text-rose-600 font-bold text-[10px] uppercase tracking-wider rounded-xl transition-all border border-transparent hover:border-rose-100"
                  ...
                  .map(course => (
                    <div key={course.id} className="bg-white p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 flex flex-col gap-4 md:gap-6 shadow-sm hover:shadow-xl transition-all group">
                      ...
                      <div className="space-y-3 md:space-y-4 px-1">
                        ...
                        {/* Live Stream, Schedule Calendar Notice & Timetable Recurrence Module */}
                        {(() => {
                          const liveState = enrolledLiveStates[course.id];
                          if (!liveState) return null;

                          return (
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100/60 space-y-2.5 text-xs font-sans">
                              {/* Timetable Recurrence Schedule */}
                              {liveState.timetable && (
                                <div className="flex items-start gap-2 text-slate-700 font-medium">
                                  ...
                                  <div>
                                    <span className="font-extrabold text-[10px] text-zinc-400 uppercase tracking-wide block">Weekly Course Timetable</span>
                                    <p className="text-slate-800 font-semibold">{liveState.timetable}</p>
                                  </div>
                                </div>
                              )}

                              {/* Live Schedule alert notifications dispatch banner */}
                              {liveState.status === 'scheduled' && liveState.scheduledTime && (
                                <div className="p-2.5 bg-rose-50 border border-rose-100/80 rounded-xl flex items-start gap-2 text-rose-950">
                                  <Clock className="h-4 w-4 text-rose-500 shrink-0 mt-0.5 animate-pulse" />
                                  <div className="space-y-0.5">
                                    <span className="font-black text-[9px] uppercase tracking-wider text-rose-700 block animate-pulse">⏰ Upcoming Scheduled Lecture</span>
                                    <p className="font-bold text-[11px] leading-tight text-rose-900">{liveState.scheduledTitle || "Next Live Session"}</p>
                                    <p className="text-[10px] text-rose-700 font-semibold">Starts on: {formatDate(liveState.scheduledTime)}</p>
                                  </div>
                                </div>
                              )}

                              {/* Study notes uploaded from Admin control panel */}
                              {liveState.notesUrl && (
                                <div className="flex items-center justify-between gap-2 p-2 bg-emerald-50/50 border border-emerald-100/60 rounded-xl mt-1">
                                  <div className="flex items-center gap-2 text-slate-700 font-semibold text-[11px] truncate">
                                    <Award className="h-4 w-4 text-emerald-600 shrink-0" />
                                    <span className="truncate">Lecture Worksheets & notes ready!</span>
                                  </div>
                                  <a
                                    href={liveState.notesUrl}
                                    target="_blank"
                                    referrerPolicy="no-referrer"
                                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-3 py-1 rounded-lg text-[9px] uppercase tracking-wider transition-colors inline-flex items-center gap-1 shrink-0"
                                  >
          ...
          {activeTab === 'live_classes' && isAdmin && (
            <motion.div 
              key="live_classes_admin"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="space-y-6"
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1 font-sans">
                  <h2 className="text-xl md:text-2xl font-bold text-slate-800 flex items-center gap-2">
                    <Video className="h-6 w-6 text-rose-500 animate-pulse" />
                    Live Class & Timetable Planner
                  </h2>
                  <p className="text-xs md:text-sm text-slate-500 font-medium font-sans">Configure live streaming classrooms, schedule recurring lectures, upload study worksheets, and broadcast notices to approved participants.</p>
                </div>
              </div>

              {/* Grid split */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-sans">
                {/* Left hand column: Choose course and hardware mic test */}
                <div className="space-y-6 lg:col-span-1">
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                    <h3 className="font-bold text-slate-700 text-sm tracking-tight border-b border-slate-50 pb-2">1. Select Target Course</h3>
                    <div className="space-y-3">
                      {courses.map(crs => {
                        const approvedStudentsCount = enrollments.filter(e => e.courseId === crs.id && e.status === 'approved').length;
                        return (
                          <button
                            key={crs.id}
                            onClick={() => setSelectedCourseForLive(crs.id)}
                            className={cn(
                              "w-full text-left p-4 rounded-2xl border transition-all flex justify-between items-center",
                              selectedCourseForLive === crs.id 
                                ? "border-rose-300 bg-rose-50/55 text-rose-950 font-semibold"
                                : "border-slate-100 hover:border-slate-200 bg-slate-50/55 hover:bg-slate-55 text-slate-700"
                            )}
                          >
                            <div className="truncate pr-2">
                              <p className="text-xs font-black truncate">{crs.title}</p>
                              <span className="text-[10px] text-slate-400 font-medium flex items-center gap-1 mt-0.5">
                                <Users className="h-3 w-3" />
                                {approvedStudentsCount} Paid & Approved Students
                              </span>
                            </div>
                            <span className={cn(
                              "h-2 w-2 rounded-full",
                              selectedCourseForLive === crs.id ? "bg-rose-500 animate-pulse scale-110" : "bg-slate-300"
                            )} />
                          </button>
                        );
                      })}
                      {courses.length === 0 && (
                        <p className="text-xs text-slate-400 text-center py-4 font-sans">No active catalog courses found. Please create one on Catalog tab first!</p>
                      )}
                    </div>
                  </div>

                  {/* Hardware Device Mic Test */}
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                    <div className="flex justify-between items-center border-b border-slate-50 pb-2">
                      <h3 className="font-bold text-slate-700 text-sm tracking-tight flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          micTestActive ? "bg-rose-100 text-rose-700 font-extrabold" : "bg-slate-100 text-slate-700 hover:bg-slate-200 font-bold"
                        )}
                      >
                        {micTestActive ? "Stop Test" : "Test Now"}
                      </button>
                    </div>

                    <p className="text-[11px] text-slate-400 leading-relaxed font-semibold">Ensure your voice sounds professional. Turn on testing, speak into your mic, and see if the live analyzer meters respond!</p>
                    ...
                    {micTestActive ? (
                      <div className="space-y-2 pt-2">
                        ...
                        <div className="flex justify-between items-center text-[9px] font-mono text-slate-400">
                          <span>Muted</span>
                          <span className="font-bold text-emerald-600 animate-pulse">Voice input registered ({micLevel}%)</span>
                          <span>Max</span>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center py-4 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                        <p className="text-xs text-slate-400 font-bold font-sans">Microphone monitor offline</p>
                      </div>
                    )}
                  </div>
                </div>

                {/* Right hand column: Configuration parameters & scheduling */}
                <div className="lg:col-span-2 space-y-6">
                  {selectedCourseForLive ? (
                    <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                      <div className="space-y-1">
                        <h3 className="font-black text-slate-800 text-base">Schedule details for:</h3>
                        <p className="text-xs font-black text-rose-500 uppercase tracking-wide">{courses.find(c => c.id === selectedCourseForLive)?.title}</p>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-1.5 space-y-1">
                          <label className="text-xs font-bold text-slate-600">Lecture / Stream Title</label>
                          <input
                            type="text"
                            placeholder="e.g. Chapter 3: Advanced Cell Nuclear Structure"
                            value={liveStreamForm.title}
                            onChange={(e) => setLiveStreamForm({...liveStreamForm, title: e.target.value})}
                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 transition-all font-medium text-slate-800"
                          />
                        </div>

                        <div className="space-y-1.5 space-y-1">
                          <label className="text-xs font-bold text-slate-600">Timetable Course Recurrence</label>
                          <input
                            type="text"
                            placeholder="e.g. Every Mon, Wed & Fri at 5:00 PM IST"
                            value={liveStreamForm.timetable}
                            onChange={(e) => setLiveStreamForm({...liveStreamForm, timetable: e.target.value})}
                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 transition-all font-medium text-slate-800"
                          />
                        </div>

                        <div className="space-y-1.5 space-y-1">
                          <label className="text-xs font-bold text-slate-600">Scheduled Date & Time</label>
                          <input
                            type="datetime-local"
                        ...
                        <div className="space-y-1.5 space-y-1">
                          <label className="text-xs font-bold text-slate-600">Worksheet / Lesson Study Notes Link</label>
                          <input
                            type="text"
                            placeholder="e.g. Link to pdf notes / worksheet document"
                            value={liveStreamForm.notesUrl}
                            onChange={(e) => setLiveStreamForm({...liveStreamForm, notesUrl: e.target.value})}
                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 transition-all font-medium text-slate-800"
                          />
                        </div>
                      </div>

                      {/* Real-time Status Controller Cockpit */}
                      <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100/80 space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <span className="text-xs font-black text-slate-700 uppercase tracking-wider block">Live Stream Status Control</span>
                            <span className="text-[10px] text-slate-400 font-semibold">Change status instantly from this panel</span>
                          </div>
                          <span className={cn(
                            "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider",
                            selectedLiveState?.status === 'live' ? "bg-rose-50 text-rose-600 border border-rose-100 animate-pulse" :
                            selectedLiveState?.status === 'scheduled' ? "bg-amber-50 text-amber-600 border border-amber-100" :
                            selectedLiveState?.status === 'ended' ? "bg-slate-100 text-slate-600 border border-slate-200" :
                            "bg-slate-50 text-slate-400 border border-slate-200"
                          )}>
                            ● {selectedLiveState?.status ? selectedLiveState.status.toUpperCase() : 'OFFLINE'}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-1">
                          <button
                            ...
                            disabled={isGenerating}
                            className={cn(
                              "py-2.5 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider border transition-all text-center",
                          ...
                          <button
                            ...
                            className={cn(
                              "py-2.5 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider border transition-all text-center",
                              selectedLiveState?.status === 'ended' 
                          ...
                          <button
                            onClick={() => handleChangeLiveStatus(selectedCourseForLive, 'scheduled')}
                            disabled={isGenerating}
                            className={cn(
                              "py-2.5 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider border transition-all text-center",
                          ...
                          <button
                            ...
                            className={cn(
                              "py-2.5 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider border transition-all text-center",
                              selectedLiveState?.status === 'idle' 
                                ? "bg-slate-200 text-slate-800 border-slate-300 shadow-md shadow-slate-100" 
                      ...
                      <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-3">
                        ...
                        <div className="space-y-1 text-slate-700">
                          <h4 className="text-xs font-extrabold uppercase tracking-wide">Approved Enrolled Members Protection Enforced</h4>
                          <p className="text-[11px] leading-relaxed font-semibold text-slate-600">This live classroom only allows entry for purchased-approved students (status is approved). General visitors or pending accounts will see high-security locks. Scheduling alerts apply exclusively to them.</p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
                        <button
                          ...
                          disabled={isGenerating}
                          className="flex-1 py-3.5 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl text-xs font-black uppercase tracking-wider shadow-lg shadow-rose-500/10 hover:shadow-rose-500/20 transition-all text-center disabled:opacity-50"
                        >
                          {isGenerating ? "Processing..." : "🗓️ Save & Schedule Session"}
                        </button>

                        <button
                          onClick={() => handleSendScheduleNotice(selectedCourseForLive)}
                          disabled={isGenerating}
                          className="flex-1 py-3.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-2xl text-xs font-black uppercase tracking-wider transition-colors text-center disabled:opacity-50"
                        >
                          📢 Notify Approved Students (Before 4 hours)
                        </button>
                      </div>

                      {/* Direct Live stream launch shortcut */}
                      <div className="pt-2">
                        <button
                          onClick={() => navigate(`/live/${selectedCourseForLive}`)}
                          className="w-full py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all text-center flex items-center justify-center gap-2 border border-slate-950 shadow-xl"
                        >
                          <Video className="h-4 w-4 text-rose-500" />
                          Launch Live Transmitter Studio (Go Live Now)
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="h-full bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-8 text-center py-24 min-h-[350px]">
                      <Video className="h-16 w-16 text-slate-300 mb-4 animate-pulse" />
                      <h4 className="font-extrabold text-slate-700 text-sm">Please Select a Course from the left side list</h4>
                      <p className="text-xs text-slate-400 max-w-sm mt-1 leading-relaxed">Select one of your catalog courses to configure its Live schedules, upload worksheets, set timetable recurrence, and alert approved students!</p>
                    </div>
                  )}
                </div>
              </div>
